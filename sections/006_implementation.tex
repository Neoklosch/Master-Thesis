...
The \textit{after\_success} block is used to create and upload the Docker images.
There are two slightly different scripts to be executed.
The first one will create and upload a normal Docker image and the second one will be used for a specific Raspberry Pi image.
A Raspberry Pi is using a \ac{CPU} with an ARM architecture.
Therefore Docker images that should be support ARM \acp{CPU} must have specific dependencies and has to be build differently.
The Raspberry Pi image for Motey has a modified Dockerfile that loads a Raspbian base image instead of an Ubuntu image.
...
Both images will only be executed if it is a master branch build (see line 17 and 22).
...
Line 18 and 23 will build the image and line 19 and 24 uploads it.
...
Finally the Docker image will be pushed in line 20 and 25.
