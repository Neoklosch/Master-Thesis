...
\begin{lstlisting}[label=code:capability-schema,caption=Capability JSON validation schema]
...
\begin{lstlisting}[label=code:config-ini,caption=Example of the config.ini file]
[GENERAL]
app_name = Motey
...
\begin{lstlisting}[label=code:configreader-example,caption=Example of the usage of the configreader]
...
Due to the implementation of the import logic in Python, the containing script will executed only once, regardless how many files imported that module.
