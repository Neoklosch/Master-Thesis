...
Figure \ref{fig:docker_container_structure} illustrates the concept for the image of the pretty famous Django\footnote{https://www.djangoproject.com/} web framework.
...
The most popular one it the official Docker Hub\footnote{https://hub.docker.com}.
There is also an Docker Store\footnote{https://store.docker.com}, where customers can buy and sell trusted and enterprise ready containers and plugins.
...
For example the already mentioned Django image can be executed and linked together with a MongoDB\footnote{https://www.mongodb.com} images.
The main benefit is the ease of configure dependencies between several containers and configuration steps.
These concept is similar to deployment tools like Vagrant\footnote{https://www.vagrantup.com}, Ansible\footnote{https://www.ansible.com} or Puppet\footnote{https://puppet.com}.
...
For Docker Kubernetes and Docker Swarm are the most popular once at the moment.
...
The user can interact with the Kubernetes system via a \ac{CLI}, an \ac{UI} or a third party application, over a \ac{REST} \ac{API} to the Kubernetes Master or more specifically the API Server in the master.
...
If for example one node disappear because of connection issues, the Repliation Controller will start a new one.
...
With rolling updates the system can update one pod at a time, rather than taking down the entire service and update the whole system.\ref[cf.]{Kubernetes:rolling-updates:2016}
This also increases the stability and availability of the system and eases the managing of container clusters.


\subsection{Network Function Virtualization}
\ac{NFV} is an architectural framework to provide \acp{NF} through software virtualization techniques to a single piece of hardware instead of multiple different devices for each function, e.g. router, firewall, storage, switch, etc.\cite[cf.][p. 8]{ETSI:NFV:2013}
...
% https://www.youtube.com/watch?v=xsd5nhG0RBs
