...
The necessary information are delivered from the monitoring system available at the \ac{NFVI} level, which can also be extended or replaced with any monitoring system by implementing a custom monitoring driver.\autocite[cf.]{openBatonDoc}
...
Therefore, it is quite important that the applications in an \ac{IoT} context use a lightweight protocol, that has less data overhead and can delivers data packages fast.
...
For example, a basic workflow could be, that a consumer subscribe to a specific topic at the broker and the producer can send messages with a specific topic to the broker.
...
The most secure, but also most resources consuming level is \textit{\ac{QoS} level 2 - exactly once}.
...
Therefore, both sides can be sure to not send or receive duplicates and it is guaranteed that a message will be received.
...
\textit{Persistent sessions} allows a connection be established, even if the consumer disappears.
In this case, all the messages incurred will be stored and delivered, if the consumer resume the session.
...
There, the \textit{Ventilator}, which produces multiple tasks, that can be done in parallel, make use of a set of worker, that can process these tasks and a sink, that collects the results from the workers.\autocite{ZeroMQ:Guide}
...
Furthermore, the works can pull a new task as soon as they are done.
...
One of the presented patterns is to use container virtualization, to split up the services and use them on low power devices.
Also in this work, Docker was used as the tool of choice.
It describes, that the usage of container virtualization enables a better testability, ease the service deployment and make the tools scale better.\autocite[cf.][p. 5]{Butzin:2016}
...
Therefore, the concept of fog and edge computing is presented as a viable solution, where low power devices, like smartphones or small \acp{PC}, are able to execute them.\autocite[cf.][p. 5]{Butzin:2016}
...
In general, edge devices like an Raspberry Pi or any other small \ac{PC}, that enables the computation and storage of data, have several sensors and actors connected to them.
...
Volumes stores data for applications that can persist data on a node.\autocite[cf.][p. 384]{Pahl:2015}
...
As mentioned before, a Raspberry Pi is powerful enough to do this job, for example to gather data at the edge of a network and to enable a communication channel with the cloud infrastructure.\autocite[cf.][p. 117]{Pahl:2016}
...
The outcome of the work was a working environment, that was able to deploy and monitor an edge cloud infrastructure.
...
Overall, this works shows the need of a solid infrastructure at the network edge to enable edge computing capabilities.
...
It can be used to enable edge nodes to discover other service provider in a network in real time to facilitate communication.\autocite[cf.][p. 34]{Stubbs:2015}
...
After the container is deployed, Serfnode can be configured to enable a service discovery mechanism that provide and can gather informations about one or multiple Docker container across a cluster without modifying the original containers.\autocite[cf.][p. 34]{Stubbs:2015}
...
Beside Serfnode, some other service discovery tools like Consul\footnote{\url{https://www.consul.io}}, Synapse\footnote{\url{http://airbnb.io/projects/synapse}} and CoreOS\footnote{\url{https://coreos.com/fleet/docs/latest/examples/service-discovery.html}} are described, with the outcome, that they are fits better in a cloud environment and are to heavy for an edge node cluster.\autocite[cf.][p. 36]{Stubbs:2015}
...
Unfortunately, the \ac{MLU} component was not described and measured results were not adequate analyzed.
...
Kura itself is an open source \ac{IoT} gateway software, that aggregate the data of multiple connected devices and can also control them.\autocite[cf.][p. 2]{Bellavista:2017}
...
Former is based on a tree hierarchies.
...
As the result Docker Swarm is used, due to the fact, that it is already integrated in Docker, it has a sufficient performance.\autocite[cf.][p. 7]{Bellavista:2017}
