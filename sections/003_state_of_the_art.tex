...
In Open Baton it is implemented as the \ac{VIM} as first \ac{PoP}.\cite{openBatonDoc}
...
In the basic configuration Open Baton provides a generic \ac{VNFM}, but it can also be replaced with a custom \ac{VNFM}.
The manager can use a \ac{REST} \ac{API} or an \ac{AMQP} message queue to communicate with the system.
Additional components, such as autoscaling and fault management, are provided to manage to the runtime of a network service.\cite{openBatonDoc}
The necessary informations are deliverd from the monitoring system available at the \ac{NFVI} level, which can also be extended or replaced by a custom monitoring system.\cite{openBatonDoc}
